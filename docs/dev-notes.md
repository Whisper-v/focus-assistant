...
| 阶段 | 内容 |
| --- | --- |
| 1. 概念设计 | 桌面陪伴型番茄钟「露」：专注 → 成长 → 情绪反馈 → 健康提醒 |
...
| 6. 窗口 | 无边框半透明圆角卡片、按住露拖动、双击开始/暂停、托盘常驻 |
...
- 视频：见提交帖（可现场演示：开始专注→露守护→撒花庆祝→休息打盹）
...
## 七、v1.1 新增：多皮肤 + 修复成长可视化

- **皮肤系统**：PetWidget 增加 `Skin`（Classic 经典粉花 / Sunflower 阳光向日葵），
  以调色板（花瓣/花盘/茎叶/表情墨色）实现纯矢量换肤，向日葵花盘另绘葵花籽点阵；设置对话框可切换并持久化。
  向日葵为**原创矢量绘制**（金色花瓣 + 棕色花盘 + 瓜子点），仅作风格参考，未使用任何第三方素材。
- **修复成长可视化 Bug**：此前 `FocusManager::stage()` 从未同步给宠物控件（`m_pet->setStage` 缺失），
  导致载入高阶段存档或中途成长时，露始终停留在种子形态；现于启动时同步一次、`stageChanged` 时实时更新。
